...
#define SEND_DATA(P) HMI_SendSingleChar(P)
#define BEGIN_CMD() TX_8(0XEE)
#define END_CMD() TX_32(0XFFFCFFFF);delay_ms(5);
...
{
	return (uint16_t)(((B)&0xf8)>>3)|(((G)&0xfc)<<3)|(((R)&0xf8)<<8);
}
void GetRTCTime()
{
	BEGIN_CMD();
...
void RTC_init(void)
{
	...
	if(RTC->BKP0R != 1 )
	{
		RTC->TR = RTC_TEN_TO_BCD(8) <<16 | RTC_TEN_TO_BCD(50) <<8|RTC_TEN_TO_BCD(52) <<0;
		RTC->DR = RTC_TEN_TO_BCD(19) <<16 | RTC_TEN_TO_BCD(1) <<13|RTC_TEN_TO_BCD(9) <<8|RTC_TEN_TO_BCD(2) <<0;
		RTC->BKP0R = 1;
	}
	RTC->ISR &=~(1<<7);
	RTC->WPR = 0XFF;
